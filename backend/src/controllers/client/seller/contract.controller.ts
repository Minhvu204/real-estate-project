...
const ensureDealForSeller = async (dealId: string, sellerId: string) => {
  const deal = await dealService.getDealForSeller(dealId, sellerId);
  if (!deal) {
...
const sendContractNotification = async (
  ...
  const agentId = getStringId(deal?.agent_id?._id || deal?.agent_id);
  const recipients = [buyerId, agentId].filter(Boolean) as string[];

  if (!recipients.length) return;

  const sellerName =
    (deal?.seller_id && deal.seller_id.fullName) ||
    (deal?.seller_id && deal.seller_id.name) ||
    "Seller";
...
export const uploadContract = async (req: Request, res: Response) => {
  try {
    ...
    const deal = await ensureDealForSeller(dealId, sellerId);
    ...
    console.error("seller.uploadContract error:", error);
...
export const replaceContract = async (req: Request, res: Response) => {
  try {
    ...
    console.error("seller.replaceContract error:", error);
    const status = error?.status || 500;
...
export const getContractByDeal = async (req: Request, res: Response) => {
  try {
    ...
    console.error("seller.getContractByDeal error:", error);
    const status = error?.status || 500;
